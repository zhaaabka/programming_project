...
То есть, если Вы укажете 25.05, то программа будет считать дедлайном 25.05.{} 23.59.'''.format(curr_day.year),
...
def add_data2(message):
    ...
    if date_s == "Без дедлайна":
        data_list.append(datetime.datetime(2222, 2, 22, 23,
                                           59))  # какое-то абстрактное далёкое будущее для дел без дедлайнов; дату можно изменить
        ...
        deadlines_to_print[new] = "{}.{}.{} {}:{}".format(thedate.day, thedate.month, thedate.year, thedate.hour,
                                                          thedate.minute)
    keyboard = types.ReplyKeyboardRemove(True)
    data3 = bot.send_message(message.from_user.id, 'Укажите важность по шкале 1-5, где 5 - наиболее важное дело.',
                             reply_markup=keyboard)
...
def dl_edit(message):
    ...
    if date_s == "Без дедлайна":
        ...
        deadlines_to_print[thing2] = "{}.{}.{} {}:{}".format(thedate.day, thedate.month, thedate.year, thedate.hour,
                                                             thedate.minute)
...
def callback_inline(call):
    for i in list(look_tasks):
        if call.data == i + 'done':
            ...
            keyboard = types.ReplyKeyboardMarkup(True)
            keyboard.row('Без дедлайна')
            new_dl = bot.send_message(call.message.chat.id, 'Введите новый дедлайн или нажмите кнопку ниже',
                                      reply_markup=keyboard)
...
def time_rem_edit(message):
    ...
    reminders[thedate] = (reminders[old_time_rem][0], "{}.{}.{} {}:{}".format(thedate.day, thedate.month, thedate.year, thedate.hour, thedate.minute))
